...
<main class="mx-auto max-w-prose px-8">
	<section id="qrcode" class="flex flex-col items-center justify-center">
		...
		{#if $contact?.phone_numbers?.length}
			{#each $contact.phone_numbers as phone, i}
				...
				>
					...
					/>
					<button class="btn --sm --icon ml-2" on:click={() => remove_phone(i)}>
						<CloseIcon />
					</button>
				</fieldset>
			{/each}
		...
		{#if $contact?.emails?.length}
			{#each $contact.emails as email, i}
				...
				>
					...
					/>
					<button class="btn --sm --icon ml-2" on:click={() => remove_email(i)}>
						<CloseIcon />
					</button>
				</fieldset>
			{/each}
		...
		{#if $contact?.urls?.length}
			{#each $contact.urls as email, i}
				...
				>
					...
					/>
					<button class="btn --sm --icon ml-2" on:click={() => remove_url(i)}>
						<CloseIcon />
					</button>
				</fieldset>
			{/each}
